    	...
    	if ((state.ball.position - state.player_state(id_team,id_player).position).norm < PLAYER_RADIUS + BALL_RADIUS):
    		if (id_team == 1):
    			if (state.ball.position.x > 112):
    				return SoccerAction(state.ball.position - state.player_state(id_team,id_player).position,(Vector2D(150,45) - state.ball.position)/3.5)
    		...
    		if (id_team == 2):
    			if (state.ball.position.x < 37):
    				return SoccerAction(state.ball.position - state.player_state(id_team,id_player).position ,(Vector2D(0,45) - state.ball.position)/3.5)
